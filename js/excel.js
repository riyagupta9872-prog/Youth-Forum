...
console.log('%c[Youth Forum] excel.js v140 loaded — template has Kirtan fields + grouped headers', 'background:#1A5C3A;color:#fff;padding:2px 8px;border-radius:3px');
...
async function exportCallingList() {
  ...
  try {
    ...
    XLSX.writeFile(wb, `youth_forum_fy${fyStartYear}_${today}.xlsx`);
...
async function exportDevoteeDatabase() {
  ...
  try {
    ...
    return _buildAndDownloadDevoteeWorkbook({
      ...
      filename: `youth_forum_database_${getToday()}.xlsx`,
    });
  } catch (e) {
    console.error(e);
...
async function _buildAndDownloadDevoteeWorkbook({ devotees, includeTeamCol, filename }) {
  try {
    ...
    if (includeTeamCol) {
      ...
      const instrRows = [
        ['YOUTH FORUM – Devotee Import Template', '', ''],
        ['', '', ''],
        ['HOW TO USE:', '', ''],
...
function downloadImportTemplate() {
  ...
  XLSX.utils.book_append_sheet(wb, wsInstr, 'Instructions');
  XLSX.writeFile(wb, 'youth_forum_devotee_template.xlsx');
  showToast('Template downloaded!', 'success');
}
